...
for i = 1:10
    for j=1:10
        [unused(i, j) results_median(i, j)] = psnr(original_image, custom_adaptive_multiplicative(noisy_image, window_size(i), multiplicative_alpha(j)));
    end
end

figure;
[X,Y] = meshgrid(1:10, 0:3:27);
surf(X, Y, results_median);
title("Edge Preserving Local Median for Multiplicative Noise - SNR vs Window Radius & Alpha");
